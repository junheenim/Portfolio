...
{    
    ...
    public GameObject smeltingLiquid;
    ...
    private void Update()
    {
        ...
        {
            // 평화상태 이동
            if (target == null && nav.remainingDistance < nav.stoppingDistance)
            {
                nav.speed = 2.0f;
                anim.SetBool("MoveWalk", false);
            }
            // 평화상태 마구 돌아다님
            if (target == null && moveAround)
            {
                moveAround = false;
    ...
    IEnumerator Die()
    {
        ...
        money.GetComponent<Coin>().coins = Random.Range(20, 30);
        int drop = Random.Range(0, 3);
        iron = Instantiate(iron);
        iron.transform.position = transform.position + new Vector3(0, 1, 0);
        if (drop == 0)
        {
            ...
            {
                smeltingLiquid = Instantiate(smeltingLiquid);
            }
        }
